...
#TODO
#put this in src/hal/usercomps
#edit plasmac.tcl to point back to usercomps
...
def write_register(reg, value):
    ...
    if len(data) == 12:
        try:
            ...
            reply = comms.readline().decode().strip()
            if reply:
                if reply == packet.strip():
                    return 1
        except:
            print('powermax communication write error')
    return 0
...
def read_register(reg):
    ...
    if len(data) == 12:
        try:
            ...
            reply = comms.readline().decode().strip()
            if reply:
                if len(reply) == 13 and reply[:7] == ':{}{}'.format(address, validRead):
                    lrc = get_lrc(reply[1:11])
                    if lrc == reply[11:13]:
                        return reply[7:11]
        except:
            print('powermax communication read error')
    return 0
