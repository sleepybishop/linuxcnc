...
For the following cases the Configuration Selector copies the chosen sample
configuration to ~/linuxcnc/configs:

* *Package install (deb, rpm, distro packages)*: sample configurations
  live under a system path and are not writabe.
...
For *Run-In-Place (RIP) builds* the source tree is normally writable by
